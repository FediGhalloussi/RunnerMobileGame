...
namespace HyperCasual.Gameplay
{
    ...
    public class SequenceManager : AbstractSingleton<SequenceManager>
    {
        ...
        void CreateMenuNavigationSequence()
        {
            ...
            splashDelay.AddLink(new Link(m_LevelStates[SaveManager.Instance.LevelProgress]));
        ...
        void CreateLevelSequences()
        {
            ...
            unloadLastScene.AddLink(new Link(m_LevelStates[SaveManager.Instance.LevelProgress]));
        }
        ...
        IState AddLevelPeripheralStates(IState loadLevelState, IState quitState, IState lastState)
        {
            ...
            var gameplayStateNoInput = new NoInputState(() => OnGamePlayStartedNoInput(loadLevelState));
            ...
            loadLevelState.AddLink(new Link(gameplayStateNoInput));

            gameplayStateNoInput.AddLink(new EventLink(m_ContinueEvent, gameplayState));
            ...
            loseState.AddLink(new EventLink(m_BackEvent, loadLevelState));
            unloadLose.AddLink(new Link(quitState));

            pauseState.AddLink(new EventLink(m_ContinueEvent, gameplayState));
            pauseState.AddLink(new EventLink(m_BackEvent, loadLevelState));
            unloadPause.AddLink(new Link(gameplayStateNoInput));
        ...
        void OnGamePlayStartedNoInput(IState current)
        {
            ...
            //todo improve
            //m_MainMenuScreen = ;
            UIManager.Instance.GetView<MainMenu>().LevelValue = SaveManager.Instance.LevelProgress;
        }
    }
}
